...
###池化
def compute_adaptive_weight(x):
    mean = torch.mean(x, dim=[2, 3], keepdim=True)
    std = torch.std(x, dim=[2, 3], keepdim=True)
    weight = (x - mean) / (std + 1e-6)
    weight = torch.clamp(weight, min=0, max=1)  # Ensure weights are non-negative and bounded
    return weight


def adaptive_soft_pool2d(x, kernel_size, stride=None, adaptive_weight=None):
    if stride is None:
        stride = kernel_size
    ...
    if adaptive_weight is not None:
        adaptive_weight = compute_adaptive_weight(adaptive_weight)
        x *= adaptive_weight

    pooled_x = F.avg_pool2d(x * e_x, kernel_size, stride=stride)
    pooled_e_x = F.avg_pool2d(e_x, kernel_size, stride=stride)

    norm_factor = torch.tensor(sum(kernel_size), dtype=x.dtype, device=x.device)
    return pooled_x * norm_factor / (pooled_e_x * norm_factor)


def Weighted_GAP(supp_feat, mask, adaptive_weight=None):
    if mask.dim() == 3:
    ...
    supp_feat *= mask
    kernel_size = (supp_feat.size(2), supp_feat.size(3))

    if adaptive_weight is not None:
        adaptive_weight = compute_adaptive_weight(adaptive_weight)

    area = adaptive_soft_pool2d(mask, kernel_size, adaptive_weight=adaptive_weight) * supp_feat.size(
        2) * supp_feat.size(3) + 1e-4
    pooled_feat = adaptive_soft_pool2d(supp_feat, kernel_size, adaptive_weight=adaptive_weight) * supp_feat.size(
        2) * supp_feat.size(3) / area
    return pooled_feat

def normalized_cosine_similarity(x1, x2, eps=1e-8):
    # 归一化的余弦相似度
...
def consist(mode, feat1, map1, mask1, feat2, map2, mask2):
    ...
    simi_map1 = normalized_cosine_similarity(feat1, vec2)
    simi_map1 = F.interpolate(simi_map1.unsqueeze(1), size=map1.size()[-2:], mode='bicubic')
    simi_map1 = simi_map1.view(map1.size())
    simi_map1 = torch.clamp(simi_map1, 0, 1)

    # 确保不会全为0或全为1
    eps = 1e-7
    simi_map1 = torch.clamp(simi_map1, eps, 1 - eps)

    if mode == 'semi_train':
        return simi_map1
    else:
        vec1 = Weighted_GAP(feat1, map1)
        simi_map2 = normalized_cosine_similarity(feat2, vec1)
        simi_map2 = F.interpolate(simi_map2.unsqueeze(1), size=map2.size()[-2:], mode='bicubic')
        simi_map2 = simi_map2.view(map2.size())
        simi_map2 = torch.clamp(simi_map2, 0, 1)

        # 确保不会全为0或全为1
        simi_map2 = torch.clamp(simi_map2, eps, 1 - eps)
